...
class hero(object):
	...
	def random_desicion(self):
		self.weight1=[[uniform(-1,1) for i in range(HERO_COUNT*2+2)] for j in range(20)]
		self.bias1=[uniform(-10,10) for i in range(20)]
		self.weight2=[[uniform(-1,1) for i in range(20)] for j in range(10)]
		self.bias2=[uniform(-10,10) for i in range(10)]
		self.weight3=[[uniform(-1,1) for i in range(10)] for j in range(16)]
		self.bias3=[uniform(-10,10) for i in range(16)]
	#-------------
	def update(self):
		...
		if self.status=="MOVING":
			...
			if self.side=="left":
				for enemy in hero.right_heroes:
					if enemy.x <= self.x+HERO_SIZE+self.hook_vector[0]*self.hook_animation_time+self.hook_size\
					   and enemy.x+HERO_SIZE >= self.x+HERO_SIZE+self.hook_vector[0]*self.hook_animation_time+self.hook_size\
					   and ((enemy.y <= self.y+HERO_SIZE/2-self.hook_size/2+self.hook_vector[1]*self.hook_animation_time\
					   and enemy.y-enemy.hook_size >= self.y+HERO_SIZE/2-self.hook_size/2+self.hook_vector[1]*self.hook_animation_time)
					 	   or (enemy.y <= self.y+HERO_SIZE/2-self.hook_size/2+self.hook_vector[1]*self.hook_animation_time+self.hook_size\
					 	   and enemy.y-enemy.hook_size >= self.y+HERO_SIZE/2-self.hook_size/2+self.hook_vector[1]*self.hook_animation_time+self.hook_size)):
					   self.score+=100
					   enemy.score-=20
				...
				for enemy in hero.right_heroes:
					if enemy.x <= self.x+self.hook_vector[0]*self.hook_animation_time-self.hook_size\
					   and enemy.x+HERO_SIZE >= self.x+self.hook_vector[0]*self.hook_animation_time-self.hook_size\
					   and ((enemy.y <= self.y+HERO_SIZE/2-self.hook_size/2+self.hook_vector[1]*self.hook_animation_time\
					   and enemy.y-enemy.hook_size >= self.y+HERO_SIZE/2-self.hook_size/2+self.hook_vector[1]*self.hook_animation_time)
					 	   or (enemy.y <= self.y+HERO_SIZE/2-self.hook_size/2+self.hook_vector[1]*self.hook_animation_time+self.hook_size\
					 	   and enemy.y-enemy.hook_size >= self.y+HERO_SIZE/2-self.hook_size/2+self.hook_vector[1]*self.hook_animation_time+self.hook_size)):
...
def draw():
	...
	for at in hero.heroes:
		if at.status != "HOOKING":
			at.desicion()
		at.update()
	...
	if FLOW:
		...
		fill(255,255,0)
		text(str(hero.heroes[0].score), screen.width/2, 30)
		text(str(L),screen.width/2-20, 80)
		text(str(R),screen.width/2-20, 250)
		text(str(hero.heroes[1].score), screen.width/2, 200)
		fill(50)
